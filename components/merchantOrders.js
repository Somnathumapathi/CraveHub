...
try{
    ...
    return (
        ...
        <h1 className="flex items-center justify-center text-4xl text-white font-bold w-3/4 hover:text-green-500 hover:scale-110 duration-300 mb-2">Orders</h1>
        {orders.map((order) => (
            <div  key={order.id} className="pt-2  w-3/4">
            <div className="pt-2 transform transition-transform bg-green-900 rounded-md pl-4">

            <p className="font-bold font-sans hover:text-green-500">CUSTOMER NAME:  {order.custName} | ORDER ID:   {(order.id).substring(0,4)}</p>
